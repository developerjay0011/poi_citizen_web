...
const ReactLikePostsFB = ({ imagesOrVideos }: ReactLikePostsFBProps) => {
  ...
  const removeImgOrVideoClone = () => {
    const elemMainContent = document.querySelector(".react-like-posts-fb-clone .main-content");
  ...
  const renderimagesOrVideos = (src: string, index: number) => {
    return (
      <div
        className={`col-${imagesOrVideos.length > 5 ? 5 : imagesOrVideos.length} ${imagesOrVideos.length > 5 && index === 5 ? "bigger-5" : ""}`}
